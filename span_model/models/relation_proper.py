...
class ProperRelationExtractor(Model):
    ...
    def _make_pair_features(self, a: torch.Tensor, b: torch.Tensor, matrix_dist,dep_feature_embeddings,a_span,b_span) -> torch.Tensor:
        ...
        if self.use_dep:
            a_dep_embeddings = []
            for i in range(0,a_span.shape[1]):
                a_start = a_span[0,i,0].item()
                a_end = a_span[0, i, 1].item() + 1
                a_embeddings = dep_feature_embeddings[:, a_start:a_end, :]
                a_mask = torch.ones((a_embeddings.shape[0], a_embeddings.shape[1]))
                a_mask = a_mask.to(a_embeddings.device)
                process_a_dep_embed = torch.sum(self.attention(a_embeddings,a_mask),dim=1).unsqueeze(0)
                a_dep_embeddings.append(process_a_dep_embed)
            a_final_dep_embed = torch.cat(a_dep_embeddings, dim=1)
            b_dep_embeddings = []
            for i in range(0,b_span.shape[1]):
                b_start = b_span[0,i,0].item()
                b_end = b_span[0, i, 1].item() + 1
                b_embeddings = dep_feature_embeddings[:,b_start:b_end,:]
                b_mask = torch.ones((b_embeddings.shape[0], b_embeddings.shape[1]))
                b_mask = b_mask.to(b_embeddings.device)
                process_b_dep_embed = torch.sum(self.attention(b_embeddings,b_mask),dim=1).unsqueeze(0)
                b_dep_embeddings.append(process_b_dep_embed)
            b_final_dep_embed = torch.cat(b_dep_embeddings, dim=1)
            a_new_span_embed = a_final_dep_embed.view(a_final_dep_embed.shape[0], a_final_dep_embed.shape[1], 1,a_final_dep_embed.shape[2]).expand(-1, -1, a_final_dep_embed.shape[1], -1)
            b_new_span_embed = b_final_dep_embed.view(b_final_dep_embed.shape[0], 1, b_final_dep_embed.shape[1], b_final_dep_embed.shape[2]).expand(-1, b_final_dep_embed.shape[1], -1, -1)
