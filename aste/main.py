...
class SpanModelDocument(BaseModel):
    ...
    def from_sentence(cls, x: Sentence, nlp):
        ...
        dep_nodes = getDepTree1(nlp_res['sentences'][0]['tokens'],
                                nlp_res['sentences'][0]['enhancedPlusPlusDependencies'])
...
class SpanModelTrainer(BaseModel):
    ...
    def name(self) -> str:
        ...
        return "_".join([self.data_name, str(self.train_kwargs["seed"])])
    ...
    def eval(self, data_split: SplitEnum) -> FScore:
        ...
        with open(path) as f:
            preds = []
            for line in f:
                pred_list = json.loads(line.strip())
                pred_list["dep"] = [[dict()],]
                preds.append(SpanModelPrediction(**pred_list))
